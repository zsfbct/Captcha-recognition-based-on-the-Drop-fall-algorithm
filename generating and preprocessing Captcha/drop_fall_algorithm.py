...
def load_labels():
	labels = []
	#LABELS_DIR="D:\交大\大四\大数据分析\验证码数据集(含大写字母)\VCR_test_data"
	...
	for f in files:
		#print (f)
		with open(os.path.join(LABELS_DIR,f), 'r',encoding = 'utf-8',errors = 'ignore') as input_file:
			#labels.extend(map(str.strip, input_file.readlines()))
			bs = input_file.readlines()
			#decode经常出现bug，F12在console里面查是utf8
			#bs_1 = map(bs, input_file.readlines()),encoding = 'utf-8'
			labels.extend(map(str.strip,bs))

                          
	#with open('data/labels.pkl', 'wb') as f:
	with open('D:/交大/大四/大数据分析/验证码数据集(含大写字母)/VCR_test_data/labels.pkl', 'wb') as f:
...
def test():
	with open('D:/交大/大四/大数据分析/验证码数据集(含大写字母)/VCR_test_data/labels.pkl', 'rb') as f:
		a = pickle.load(f)
	print (a.shape)
	print (a[500:510])
